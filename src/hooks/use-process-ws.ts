...
export interface UseProcessWebSocketReturn {
...
function buildWsUrl(configId: string): string {
  const loc = window.location;
  const protocol = loc.protocol === 'https:' ? 'wss:' : 'ws:';
  const token = localStorage.getItem('access_token') ?? '';
  return `${protocol}//${loc.host}/api/v1/processes/${configId}/run/ws?token=${encodeURIComponent(token)}`;
}

export function useProcessWebSocket(configId: string): UseProcessWebSocketReturn {
